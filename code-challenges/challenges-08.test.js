...
function returnTen(str) {
  ...
  return splittedString.splice(-10);
}
...
const getStatName = (arr, minBaseStat) => {
  ...
  let filteredArray = arr.filter((element) => {
    // console.log(Number.isInteger(element))
    // element.baseStat > minBaseStat ? element.stat.name : console.log('');
    if (element.baseStat > minBaseStat) {
      return element.stat.name;
    }
  });
  let filteredmappedArray = filteredArray.map((element) => {
    return element.stat.name;
  });
