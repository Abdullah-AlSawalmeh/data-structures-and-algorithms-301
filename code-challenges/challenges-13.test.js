...
const longestString = (arr) => {
  // Solution code here...
  return arr.indexOf(
    arr.reduce((acc, cur) => (acc.length > cur.length ? acc : cur), arr[0])
  );
};
...
const onlyOddChars = (str) => {
  // Solution code here...
  let arr = str.split("");
  return arr.filter((element) => arr.indexOf(element) % 2 === 1).join("");
};
...
const characterByIndex = (arr) => {
  // Solution code here...
  return arr.map((element, i) => element.slice(i, i + 1));
};
