...
const sortByChildren = (charArray) => {
  // Solution code here...
  charArray.sort((a, b) => {
    let cheLenA = a.children.length;
    let cheLenB = b.children.length;
    console.log(cheLenA);
    if (cheLenA < cheLenB) {
      return -1;
    }
    ...
    if (cheLenA === cheLenB) {
      let houNamA = a.name[0];
      let houNamB = b.name[0];
      if (houNamA < houNamB) {
        return -1;
      }
      if (houNamA > houNamB) {
...
const checkValues = (obj, value) => {
  // Solution code here...
  let ojbValues = Object.values(obj);
  return ojbValues.includes(value);
};
...
const updateNumbers = (obj) => {
  // Solution code here...
  let myArray = [];
  let ojbEnt = Object.entries(obj);
  ojbEnt.forEach((element) => {
    myArray.push(`${element[0]}: ${element[1]}`);
  });
  // let xx = ojbEnt.split("")
  // console.log(xx);
  return myArray;
...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach((element) => {
    ...
    if (myArray.includes(character)) {
      if (element.children.length) {
        hasChildren.push(element.name);
      }
    }
  });
  if (hasChildren.includes(character)) {
    return true;
  } else {
    return false;
  }
