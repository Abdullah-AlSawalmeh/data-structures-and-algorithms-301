...
const addTwo = (arr) => {
  ...
  let newArray = [];
  arr.forEach((element) => {
    newArray.push(Number(element) + 2);
  });
  return newArray;
};
...
let hangman = (str) => {
  // Solution code here...
  let result = str.replace(/[aeiouAEIOU]/g, "_");
  return result;
};
...
const findShells = (str) => {
  // Solution code here...
  const found = str.match(/\b\w+(ells)\b/g);
  return found;
};
